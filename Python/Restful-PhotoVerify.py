...
file_name3 = os.path.expanduser("~/idphoto.jpg")

# Check if the image files exist
if not os.path.isfile(file_name1) or not os.path.isfile(file_name2) or not os.path.isfile(file_name3):
    print("Error: One or all image files do not exist in the path {}.".format(os.path.expanduser('~')))
    exit(1)
...
image3 = encode_image(file_name3)
...
response = requests.post("{}/api/v1/photoverify".format(bws_endpoint), json=payload, headers=headers)
